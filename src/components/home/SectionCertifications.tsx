...
const SectionCertifications = ({ id }: { id: string }) => {
  const homepageCerts = certifications.filter(
    (c) => c.keepOnHomepage !== false
  );
  const categories = Array.from(new Set(homepageCerts.map((c) => c.category)));
      ...
      <ConstrainedBox classNames="p-4 py-16">
        ...
        <div className="flex flex-col gap-12 mt-8">
          {categories.map((category) => (
            <div key={category}>
              <h3 className="text-2xl font-bold mb-6 text-[var(--textColor)]">
                {category}
              </h3>
